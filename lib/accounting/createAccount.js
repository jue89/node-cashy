...
module.exports = function( db, data ) {
	...
	return parentCheck.then( ( res ) => {
		...
		if( res.cnt !== 1 ) return Promise.reject(
			new Error( `Parent account '${data.id.substr(0,data.id.indexOf('/'))}' is missing, opened in the future or closed` )
		);
		...
		return db.run( 'INSERT INTO accounts (id,dateOpened,description,data) VALUES (?,?,?,?);', [
			...
			if( e.code == 'SQLITE_CONSTRAINT' ) return Promise.reject(
				new Error( "Account id already existing" )
			);
			return e;
		} );
