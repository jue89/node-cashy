...
function Accounting (opts) {
	...
	obj(this).setHidden({ _invert: new RegExp(opts.invert) });
	delete opts.accuracy;
	delete opts.invert;

	// Connect to database
	obj(this).setHidden({ _db: dbFactory(opts) });
	...
	this._db.then((db) => {
		obj(this)
			.setImmutable({ accuracy: db.appidMinor })
			.setHidden({ _accuracy: Math.pow(10, this.accuracy) })
			.setHidden({ _maxint: Number.MAX_SAFE_INTEGER / this._accuracy });
	}).catch(() => {});
};
